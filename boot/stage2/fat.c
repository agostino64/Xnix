...
bool FAT_Initialize(DISK* disk)
{
    ...
    {
        printb("FAT: read boot sector failed\r\n");
        return false;
    }
    ...
    {
        printb("FAT: not enough memory to read FAT! Required %lu, only have %u\r\n", sizeof(FAT_Data) + fatSize, MEMORY_FAT_SIZE);
        return false;
    }

    if (!FAT_ReadFat(disk))
    {
        printb("FAT: read FAT failed\r\n");
        return false;
    }
    ...
    {
        printb("FAT: read root directory failed\r\n");
        return false;
    }
...
FAT_File* FAT_OpenEntry(DISK* disk, FAT_DirectoryEntry* entry)
{
    ...
    if (handle < 0)
    {
        printb("FAT: out of file handles\r\n");
        return false;
    }
    ...
    {
        printb("FAT: open entry failed - read error cluster=%u lba=%u\n", fd->CurrentCluster, FAT_ClusterToLba(fd->CurrentCluster));
        for (int i = 0; i < 11; i++)
            printb("%c", entry->Name[i]);
        printb("\n");
        return false;
    }
...
uint32_t FAT_Read(DISK* disk, FAT_File* file, uint32_t byteCount, void* dataOut)
{
    ...
    while (byteCount > 0)
    {
        ...
        // printb("leftInBuffer=%lu take=%lu\r\n", leftInBuffer, take);
