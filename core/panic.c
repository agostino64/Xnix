...
void panic(registers_t* regs, const char* message) 
{
    ...
    if (regs) {
        ...
        KLOG(LOG_LEVEL_INFO, " EAX: 0x%x  EBX: 0x%x  ECX: 0x%x  EDX: 0x%x\n", regs->eax, regs->ebx, regs->ecx, regs->edx);
        KLOG(LOG_LEVEL_INFO, " ESI: 0x%x  EDI: 0x%x  EBP: 0x%x  ESP: 0x%x\n", regs->esi, regs->edi, regs->ebp, regs->esp);
        KLOG(LOG_LEVEL_INFO, " EIP: 0x%x  CS:  0x%x  EFLAGS: 0x%x\n", regs->eip, regs->cs, regs->eflags);
        KLOG(LOG_LEVEL_INFO, " USERESP: 0x%x  SS: 0x%x\n", regs->useresp, regs->ss);
        KLOG(LOG_LEVEL_INFO, " INT#: %u  ERR_CODE: 0x%x\n", regs->int_no, regs->err_code);
        KLOG(LOG_LEVEL_INFO, "========================\n");
    } else {
