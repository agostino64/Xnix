...
// --- INFO, WARN, ERROR logging controlled by LOG_LEVEL ---
  ...
  #define KERN_DEBUG(format, ...) \
    printk_serial("DEBUG: %s [%s:%d]: " format, __FILE__, __func__, __LINE__, ##__VA_ARGS__)
#else
  ...
  #define KERN_INFO(format, ...) \
    printk_serial("INFO:  " format, ##__VA_ARGS__)
#else
  ...
  #define KERN_WARN(format, ...) \
    printk_serial("WARN:  %s [%s:%d]: " format, __FILE__, __func__, __LINE__, ##__VA_ARGS__)
#else
  ...
  #define KERN_ERROR(format, ...) \
    printk_serial("ERROR: %s [%s:%d]: " format, __FILE__, __func__, __LINE__, ##__VA_ARGS__)
#else
